...
(function () {
  ...
  (function render() {
    ...
    el.addEventListener('click', e => {
      ...
      const targetGrid = el.querySelectorAll('.schedule-grid')[idx];
...
(function renderReviews() {
  ...
  function launch(list, rating, total) {
    ...
    list.forEach(function (r, i) {
      ...
      dateSpan.textContent = 'Reseña de Google';
      ...
      starsDiv.innerHTML = starsFull; // SVG constante hardcodeado, sin datos externos

      const textP = document.createElement('p');
      textP.className = 'review-text';
      textP.textContent = String(r.text);
...
(function () {
  ...
  photos.forEach((_, i) => {
    ...
    d.setAttribute('aria-label', 'Foto ' + (i + 1) + ' de ' + n);
  ...
  const lbFocusable = ['flb-close', 'flb-prev', 'flb-next'].map(id => document.getElementById(id));
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.key === 'ArrowLeft') { goTo(current - 1); openLightbox(); return; }
    if (e.key === 'ArrowRight') { goTo(current + 1); openLightbox(); return; }
    if (e.key === 'Tab') {
...
// ── Form logic ────────────────────────────────────
(function () {
  const FORM_PROXY_URL = 'backend/form-proxy.php';
  ...
  // Campos obligatorios — lesión y enfermedad son opcionales
  ...
  // Todos los campos para la barra de progreso
  const allFields = [
    ...requiredFields,
  ...
  function isValidPhone(v) { return /^\+?[\d\s\-]{9,15}$/.test(v.trim()); }
  ...
  function updateProgress() {
    ...
    const reqReady = requiredFields.every(isFilled) && rgpdCheck && rgpdCheck.checked;

    submitBtn.disabled = !reqReady;
    if (reqReady) submitBtn.classList.add('su-ready');
    else          submitBtn.classList.remove('su-ready');

    notice.classList.remove('su-notice--error');

    if (reqReady) {
      const tarifaEl  = document.getElementById('su-tarifa');
      const tarifaStr = tarifaEl && tarifaEl.selectedIndex > 0
        ? tarifaEl.options[tarifaEl.selectedIndex].text : 'tu tarifa';
      // Construir el aviso de forma segura sin innerHTML
      noticeText.textContent = '';
      const wLink = document.createElement('a');
      wLink.href = 'https://drakkar.wodbuster.com/';
      wLink.target = '_blank';
      wLink.rel = 'noopener noreferrer';
      wLink.style.cssText = 'color:var(--gold);text-decoration:underline';
      wLink.textContent = 'WodBuster';
      const boldTarifa = document.createElement('strong');
      boldTarifa.textContent = tarifaStr;
      noticeText.append('¡Todo listo! Pulsa el botón para registrar tus datos. Después, crea tu cuenta en ', wLink, ' con este mismo correo y ven al box a abonar ', boldTarifa, '.');
  ...
  allFields.forEach(f => {
    f.addEventListener('input',  function () { f.removeAttribute('aria-invalid'); updateProgress(); });
    f.addEventListener('change', function () { f.removeAttribute('aria-invalid'); updateProgress(); });
  });
  ...
  function showError(msg, fieldEl) {
    ...
    notice.classList.add('show', 'su-notice--error');
    submitBtn.classList.remove('su-sending');
    submitBtn.textContent = 'Enviar inscripción';
    const canSubmit = requiredFields.every(isFilled) && rgpdCheck && rgpdCheck.checked;
    submitBtn.disabled = !canSubmit;
    if (canSubmit) submitBtn.classList.add('su-ready');
    else           submitBtn.classList.remove('su-ready');
  }

  function showFinal(nombre, tarifa) {
    form.style.display = 'none';
    notice.classList.remove('show');
    sucTitle.textContent = '¡Inscripción registrada, ' + nombre + '!';
    // Construir mensaje de éxito de forma segura sin innerHTML
    sucBody.textContent = '';
    const wLink = document.createElement('a');
    wLink.href = 'https://wodbuster.com/account/login.aspx';
    wLink.target = '_blank';
    wLink.rel = 'noopener noreferrer';
    wLink.style.cssText = 'color:var(--gold);text-decoration:underline';
    wLink.textContent = 'wodbuster.com';
    const boldDir = document.createElement('strong');
    boldDir.textContent = 'C/ Gravera nº8, Huelva';
    const boldTarifa = document.createElement('strong');
    boldTarifa.textContent = tarifa;
    sucBody.append('Paso 2: crea tu cuenta en ', wLink, ' usando el correo que introduciste. Después acércate al box (', boldDir, ') y abona ', boldTarifa, ' a uno de los entrenadores.');
  ...
  form.addEventListener('submit', function (e) {
    e.preventDefault();
    if (sending) return;
    if (!requiredFields.every(isFilled) || !rgpdCheck || !rgpdCheck.checked) return;

    // Validación de formato
    const correoEl   = document.getElementById('su-correo');
    const telefonoEl = document.getElementById('su-telefono');
    if (!isValidEmail(correoEl.value)) {
      showError('Introduce un correo electrónico válido.', correoEl);
      correoEl.focus();
      return;
    }
    if (!isValidPhone(telefonoEl.value)) {
      showError('Introduce un teléfono válido (mínimo 9 dígitos).', telefonoEl);
      telefonoEl.focus();
      return;
    }
    ...
    const tarifaEl = document.getElementById('su-tarifa');
    const tarifa   = tarifaEl.selectedIndex > 0 ? tarifaEl.options[tarifaEl.selectedIndex].text : '';

    const hpEl = document.getElementById('su-hp');
    const payload = {
      nombre:     nombre,
      apellidos:  document.getElementById('su-apellidos').value.trim(),
      telefono:   telefonoEl.value.trim(),
      correo:     correoEl.value.trim(),
      tarifa:     tarifaEl.value,
    ...
    fetch(FORM_PROXY_URL, {
      ...
      .then(function (response) {
        sending = false;
        if (response.ok) {
          showFinal(nombre, tarifa);
        } else {
          return response.json().then(function (data) {
            showError(data.error || 'No se pudo registrar la inscripción. Inténtalo de nuevo o contáctanos por WhatsApp: 604 95 57 06.');
          }).catch(function () {
            showError('No se pudo registrar la inscripción (error ' + response.status + '). Inténtalo de nuevo o contáctanos por WhatsApp.');
          });
        }
      })
      .catch(function () {
        sending = false;
        showError('Error de conexión. Comprueba tu internet o escríbenos por WhatsApp: 604 95 57 06.');
      });
  });
...
(function () {
  ...
  callout.addEventListener('keydown', function (e) {
...
// ── Chatbot Drakkar Box ───────────────────────────
(function () {
  const WEBHOOK = 'backend/chat-proxy.php';
  let open = false;
  let thinking = false;
  ...
  window.dkSend = async function (text) {
    ...
    try {
      const res = await fetch(WEBHOOK, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: msg, conversationId: convId, history: history }),
      });
  ...
  // Renderiza texto con soporte de **negrita** y saltos de línea usando solo DOM
  function dkRender(container, text) {
...
(function () {
  ...
  function togglePrice(card) {
    ...
    if (!isOpen) {
      card.classList.add('price-open');
      card.setAttribute('aria-expanded', 'true');
    }
  }

  function syncPriceTabindex() {
    var isMobile = window.innerWidth <= BP;
    document.querySelectorAll('.price-card, .addon-card').forEach(function (c) {
      if (isMobile) {
        c.setAttribute('tabindex', '0');
        c.setAttribute('aria-expanded', c.classList.contains('price-open') ? 'true' : 'false');
      } else {
  ...
  syncPriceTabindex();
  window.addEventListener('resize', syncPriceTabindex, { passive: true });
})();
...
(function () {
  ...
  function syncActivityTabindex() {
    var isMobile = window.innerWidth <= BP;
    document.querySelectorAll('.activity-card:not(.featured) .act-header').forEach(function (header) {
      if (isMobile) {
        header.setAttribute('tabindex', '0');
        var card = header.closest('.activity-card');
        header.setAttribute('aria-expanded', card && card.classList.contains('act-open') ? 'true' : 'false');
      } else {
  ...
  syncActivityTabindex();
  window.addEventListener('resize', syncActivityTabindex, { passive: true });
})();
...
// ── Cookie consent ────────────────────────────────
(function () {
  const banner = document.getElementById('dk-cookie-banner');
  if (!banner) return;
  if (localStorage.getItem('dk_cookie_consent')) return;
  banner.hidden = false;
  document.getElementById('dk-cookie-accept').addEventListener('click', function () {
    localStorage.setItem('dk_cookie_consent', '1');
    banner.hidden = true;
  });
})();
