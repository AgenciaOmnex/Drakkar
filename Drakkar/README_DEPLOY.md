...
| `GOOGLE_SHEETS_URL` | `form-proxy.php` | Usa URL hardcodeada de fallback en el PHP |
| `N8N_WEBHOOK_URL` | `chat-proxy.php` | Usa URL hardcodeada de fallback en el PHP |
| `N8N_CHAT_TOKEN` | `chat-proxy.php` | Usa token hardcodeado de fallback en el PHP |
| `ALLOWED_ORIGIN` | los tres proxies | Default `https://drakkarbox.com` (correcto) |
...
ALLOWED_ORIGIN=https://drakkarbox.com
```

> Los tres proxies tienen valores de fallback hardcodeados por si el hosting no
> soporta variables de entorno PHP. Sin embargo, si Hostinger las soporta
> (lo hace en planes Business+), configúralas para no exponer secretos en código.
...
|--------|---------|--------|
| Secretos hardcodeados como fallback en PHP | Bajo (solo server-side) | Opcional: configurar env vars en hPanel |
